...
function Test-BinaryPaths {
    ...
    try {
        ...
        Write-Host "SUCCESS: Config loaded successfully" -ForegroundColor Green
    }
    catch {
    ...
    Write-Host ""
    Write-Host "Testing binary paths:" -ForegroundColor Yellow
        ...
        if (Test-Path $binaryPath -PathType Leaf) {
            ...
            try {
                ...
                if ($process.ExitCode -eq 0 -or $process.ExitCode -eq 1) {
                    Write-Host "SUCCESS: $binaryName - Valid and executable" -ForegroundColor Green
                    $validCount++
                } else {
                    Write-Host "WARNING: $binaryName - File exists but may not be executable" -ForegroundColor Yellow
                }
            }
            catch {
                Write-Host "ERROR: $binaryName - File exists but execution failed" -ForegroundColor Red
                
                # Try to find corrected path
                $correctedPath = Find-BinaryCorrection -BinaryName $binaryName
                if ($correctedPath) {
                    Write-Host "FOUND: Correction available: $correctedPath" -ForegroundColor Cyan
                    $corrections[$binaryName] = $correctedPath
                }
            }
        } else {
            Write-Host "ERROR: $binaryName - File not found" -ForegroundColor Red
    ...
    if ($UpdateConfig -and $corrections.Count -gt 0) {
        Write-Host ""
        Write-Host "Updating config with corrected paths..." -ForegroundColor Yellow
        ...
        Write-Host "SUCCESS: Config updated successfully" -ForegroundColor Green
    }
    ...
    if ($corrections.Count -gt 0) {
        Write-Host ""
        Write-Host "Corrections:" -ForegroundColor Cyan
        foreach ($correction in $corrections.GetEnumerator()) {
